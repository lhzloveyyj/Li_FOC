...
void comm_task_func(void *pvParameters)
{
  ...
  /* add user code begin comm_task_func 2 */
    ...
    /* ---- PID 参数 ---- */
    /* 电流环 PID 从 Flash 加载，掉电不丢失；首次无数据时使用默认值 */
    SetCurrentPIDParams(g_pMotor, g_readback.current_pid_kp, g_readback.current_pid_ki,
                        g_readback.current_pid_kd, g_readback.current_pid_out);
